...
- `drive.sh` adds read-only Google Drive activity and recall helpers: `auth`, `status`, `recent [days]`, `recent [days] --all`, `recall [query...]`, and `read <id>`.
...
- **Core Mechanics:** `coach_ops.sh` validates tools. `coach_metrics.sh`, `coach_brief.sh`, `coach_prompts.sh`, `coach_prebrief.sh`, and `coach_scoring.sh` handle metric collection, deterministic brief rendering, framing prompts, pre-brief questions, and state logging.
...
- **Context Gathering:** The coach consumes a wide array of signals — manual energy/fog checks, Fitbit metrics, spoon usage, daily focus text, recent GitHub activity, and focus-related Drive evidence — all summarized into the deterministic brief that grounds the AI framing layer.
- **Strategy Evidence:** While Git is the strongest code-day signal, the behavior digest also counts focus-related journal hits and relevant Google Drive docs (`drive.sh recent`) as valid strategy evidence. It also captures broader recent Drive file activity (`drive.sh recent --all`) as context without treating unrelated files as focus alignment. When a top recent Google editor doc is available, the coach prompt can include a cached Drive excerpt without doing an extra network fetch during prompt assembly.
- **Repo Safety:** `goodevening` runs a project safety scan and surfaces findings with a detail cap so the wrap-up stays focused.
