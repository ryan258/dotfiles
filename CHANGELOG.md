...
- Added `scripts/repo_tracker.sh` plus a `github_inactive_repos.txt` state file so repos can be deactivated when they are in a good place, hidden from daily GitHub activity signals, and listed separately for later reactivation. The inactive-repo filter now loads that parked set once per helper call, and `repo_tracker.sh reactivate` reports a no-op cleanly when the repo is already active.
