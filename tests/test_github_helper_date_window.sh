...
@test "_utc_window_for_local_date uses next local midnight on DST fall-back day" {
    run bash -c "source '$DOTFILES_DIR/scripts/lib/date_utils.sh'; source '$WINDOW_FUNC_FILE'; export TZ='America/Los_Angeles'; _utc_window_for_local_date '2026-11-01'"
    [ "$status" -eq 0 ]

    start_epoch="$(printf '%s\n' "$output" | sed -n '3p')"
    end_epoch="$(printf '%s\n' "$output" | sed -n '4p')"

    run bash -c "source '$DOTFILES_DIR/scripts/lib/date_utils.sh'; export TZ='America/Los_Angeles'; date_shift_from '2026-11-01' 0 '%s'; date_shift_from '2026-11-02' 0 '%s'"
    [ "$status" -eq 0 ]
    ...
    [ "$start_epoch" -eq "$expected_start" ]
    [ "$end_epoch" -eq "$expected_end" ]
    [ $((end_epoch - start_epoch)) -eq 90000 ]
}
